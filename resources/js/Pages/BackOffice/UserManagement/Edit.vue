<template>
  <dashboard-layout title="Edit User">
    <template #header>
      <div class="flex justify-between sm:px-6 lg:px-0">
        <h1 class="text-2xl font-semibold text-gray-900">
          Edit User: {{ user.name }}{{ currentClient ? ` (${currentClient.name})` : '' }}
        </h1>
          ...
          <div class="bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
            <div class="md:grid md:grid-cols-3 md:gap-6">
              <div class="md:col-span-1">
                ...
                <p class="mt-1 text-sm text-gray-500">Basic information about the user account.</p>
              </div>
...
const handleSubmit = () => {
  ...
  const routeParams = clientId ? { user: props.user.id, client_id: clientId } : { user: props.user.id }
  
  form.put(route('back-office.users.update', routeParams))
}
